...
from data.dataset_custumer import CustomTrainDataset
...
def defaultdict_from_json(jsonDict):
    # JSON文件加载的普通Python字典转换为一个defaultdict。
    # 这样做是为了增强字典的健壮性，使得在访问字典中不存在的键时，代码不会因为KeyError而中断，而是返回一个默认的字符串值。
...
def train(training_dataset_loader, testing_dataset_loader, args, data_len, sub_class, class_type, device):
    ...
    # 检查是否需要加载预训练模型
    if args["resume"] == True:
        model_path = f'{args["output_path"]}/model/diff-params-ARGS={args["arg_num"]}/{sub_class}/params-best.pt'
        ...
        print(f"Loaded pre-trained models, optimizers, and scheduler from {model_path}")
    else:
        epoch = 0
        best_loss = float('inf')

    tqdm_epoch = range(epoch, epoch + args['EPOCHS'])
    ...
    for epoch in tqdm_epoch:
        ...
        # 记录损失到TensorBoard
        tb_writer.add_scalar('Train Loss', train_loss / len(training_dataset_loader), epoch + 1)
        tb_writer.add_scalar('Train Focal Loss', train_focal_loss / len(training_dataset_loader), epoch + 1)
        tb_writer.add_scalar('Train Smooth L1 Loss', train_smL1_loss / len(training_dataset_loader), epoch + 1)
        tb_writer.add_scalar('Train Noise Loss', train_noise_loss / len(training_dataset_loader), epoch + 1)
        ...
        # 根据当前损失和之前的最佳损失判断是否保存最佳模型
        current_loss = train_loss / len(training_dataset_loader)
        if current_loss < best_loss:
            best_loss = current_loss
            save(unet_model, seg_model, optimizer_ddpm, optimizer_seg, 
                 scheduler_seg, args, final='best', epoch=epoch, 
                 sub_class=sub_class, loss=current_loss)
        # 每隔50轮判断是不是最佳模型，如果是则保存
    ...
    # Log best performance to TensorBoard
    tb_writer.add_scalar('Performance/Best_Image_AUROC', best_image_auroc, best_epoch)
    tb_writer.add_scalar('Performance/Best_Pixel_AUROC', best_pixel_auroc, best_epoch)
                
    save(unet_model, seg_model, optimizer_ddpm, optimizer_seg, 
         scheduler_seg, args, final='last', epoch=args['EPOCHS'], 
         sub_class=sub_class, loss=train_loss / len(training_dataset_loader))
...
def save(unet_model, seg_model, optimizer_ddpm, optimizer_seg, scheduler_seg, args, final, epoch, sub_class, loss):
    checkpoint = {
    ...
    model_path = f'{args["output_path"]}/model/diff-params-ARGS={args["arg_num"]}/{sub_class}/params-{final}.pt'
...
def main():
    ...
    for sub_class in current_classes:    
        ...
        training_dataset_loader = DataLoader(training_dataset, batch_size=args['Batch_Size'],shuffle=True,num_workers=0,pin_memory=True,drop_last=True)
        test_loader = DataLoader(testing_dataset, batch_size=1,shuffle=False, num_workers=0)
