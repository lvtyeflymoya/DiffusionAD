...
class MVTecTrainDataset(Dataset):
    ...
    def perlin_synthetic(self, image, thresh, anomaly_source_path, cv2_image,thresh_path):
        ...
        image - 原始图像
        thresh - 阈值
        anomaly_source_path - 异常源图像路径
        cv2_image - 使用OpenCV读取的图像
        thresh_path - 阈值图像路径 
        """
        ...
        if no_anomaly > 0.5:
            ...
            if self.classname in texture_list: # only DTD
                ...
                # 屏蔽DTD数据集异常合成
                if texture_or_patch > 1:  # >1 is DTD 
                    aug = self.randAugmenter()
                    anomaly_source_img = cv2.cvtColor(cv2.imread(anomaly_source_path),cv2.COLOR_BGR2RGB)
                    anomaly_source_img = cv2.resize(anomaly_source_img, dsize=(
                        self.resize_shape[1], self.resize_shape[0]))
    ...
    def __getitem__(self, idx):
        ...
        # 将anomaly_source改成与thresh匹配
        thresh_file_name = os.path.splitext(os.path.basename(thresh_path))[0]
        anomaly_path = os.path.join(self.anomaly_source_path, f"{thresh_file_name}.jpg")
