...
def get_beta_schedule(num_diffusion_steps, name="cosine"):
    ...
    if name == "cosine":
        ...
        # 如果名称不在支持的调度方法中，抛出未实现错误
        raise NotImplementedError(f"unknown beta schedule: {name}")
    return betas


def extract(arr, timesteps, broadcast_shape, device):
    # 将numpy数组转换为torch张量，并根据timesteps选择特定的时间步，最后转换为浮点型
    res = torch.from_numpy(arr).to(device=timesteps.device)[timesteps].float()
    # 如果res的维度小于broadcast_shape的维度，则在res的末尾添加维度，直到维度匹配
    while len(res.shape) < len(broadcast_shape):
        res = res[..., None]
    # 将res扩展到broadcast_shape的形状，并移动到指定的device
